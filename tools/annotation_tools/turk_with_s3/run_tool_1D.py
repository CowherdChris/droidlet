...
rc = subprocess.call(
    [
        "python3 create_jobs.py --xml_file fetch_question_D.xml --tool_num 4 --input_csv D/turk_input.csv --job_spec_csv D/turk_job_specs.csv {}".format(dev_flag)
    ],
    shell=True,
)
if rc != 0:
...
rc = subprocess.call(
    [
        "python3 get_results.py --output_csv D/turk_output.csv {}".format(dev_flag)
    ],
    shell=True
)
if rc != 0:
...
rc = subprocess.call(["python3 collate_answers.py --turk_output_csv D/turk_output.csv --job_spec_csv D/turk_job_specs.csv --collate_output_csv D/processed_outputs.csv"], shell=True)
if rc != 0:
